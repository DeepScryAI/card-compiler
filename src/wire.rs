//! Scryfall CDN image-URL computation (shared core for ds-722 / task #7).
//!
//! Both the browser client (Prong A: a compact name→(uuid, version) table
//! shipped as a hashed CAS asset) and `deepscry download` (Prong B: local /images
...
//! computation compiles for the native `deepscry download` build AND the wasm
//! client — one implementation, no Rust/JS drift (DRY, task #7).

/// Scryfall CDN host (always https). This is the `base` of the DEFAULT
/// [`UrlScheme`] ([`SCRYFALL_SCHEME`]); the size segment + `front/` fan-out +
/// `.jpg` extension are appended by [`cdn_image_url_with`].
...
pub enum CdnSize {
    ...
    /// 488×680 — detail view.
    Normal,
    /// Cropped artwork only (no card frame/text); native aspect varies per
    /// card. Used by the native web GUI's "art-only" render mode, which draws
    /// our own minimal title/PT chrome over/under this crop.
    ArtCrop,
}

impl CdnSize {
    /// The CDN path + `image_uris` key segment ("small" / "normal" /
    /// "art_crop"). All three are real Scryfall CDN size segments.
    pub fn segment(self) -> &'static str {
        match self {
            CdnSize::Small => "small",
            CdnSize::Normal => "normal",
            CdnSize::ArtCrop => "art_crop",
        }
    }
}

/// Which image index a URL is reconstructed against — its CDN host `base` and
/// file `ext`ension. A scheme is a property of WHICH index a table came from:
/// the stock Scryfall index reconstructs `cards.scryfall.io/<…>.jpg` URLs
/// ([`SCRYFALL_SCHEME`]), whereas an R2-hosted AI-art index reconstructs e.g.
/// `cdn.deepscry.net/art_crop/<…>.png` URLs with the SAME UUID fan-out shape.
///
/// Strongly-typed (not a `(String, String)`): callers pass a `UrlScheme`, never
/// loose strings. Fields are `&'static str` because every real scheme is a
/// compile-time constant, keeping the type `Copy` and allocation-free.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub struct UrlScheme {
    /// CDN host + optional leading path, WITHOUT a trailing slash
    /// (e.g. `"https://cards.scryfall.io"` or `"https://cdn.deepscry.net/art_crop"`).
    pub base: &'static str,
    /// File extension WITHOUT the leading dot (e.g. `"jpg"` or `"png"`).
    pub ext: &'static str,
}

/// The DEFAULT scheme: the stock immutable Scryfall CDN (`cards.scryfall.io`,
/// `.jpg`). Every legacy caller of [`cdn_image_url`] resolves against this, and
/// it is the default scheme for a freshly-decoded [`CardLookupTable`].
pub const SCRYFALL_SCHEME: UrlScheme = UrlScheme {
    base: SCRYFALL_CDN,
    ext: "jpg",
};

impl Default for UrlScheme {
    /// Defaults to [`SCRYFALL_SCHEME`] so a `#[derive(Default)]` `CardLookupTable`
    /// reconstructs stock Scryfall URLs (back-compat with the pre-scheme world).
    fn default() -> Self {
        SCRYFALL_SCHEME
    }
}

/// Build the immutable CDN image URL for `(scryfall_id, version)` at `size`,
/// against an explicit [`UrlScheme`]. `version` is the bare cache-buster digits
/// (see module docs); pass it WITHOUT a leading `?`.
///
/// The path shape — `<base>/<size>/front/<a>/<b>/<id>.<ext>?<version>` — is
/// shared by every scheme (the curator deliberately mirrors Scryfall's fan-out;
/// see `ai_docs/transient/ART_INDEX_CONSUMPTION_PLAN_20260615.md`), so only the
/// host `base` and file `ext` vary between indices.
///
...
/// already tolerates).
pub fn cdn_image_url_with(scheme: UrlScheme, scryfall_id: &str, version: &str, size: CdnSize) -> String {
    let mut chars = scryfall_id.chars();
    let a = chars.next().unwrap_or('0');
    let b = chars.next().unwrap_or('0');
    format!(
        "{}/{}/front/{a}/{b}/{scryfall_id}.{}?{version}",
        scheme.base,
        size.segment(),
        scheme.ext,
    )
...
/// `size`. Thin wrapper over [`cdn_image_url_with`] pinned to the default
/// [`SCRYFALL_SCHEME`] — kept byte-identical for all existing call sites.
pub fn cdn_image_url(scryfall_id: &str, version: &str, size: CdnSize) -> String {
    cdn_image_url_with(SCRYFALL_SCHEME, scryfall_id, version, size)
}
...
// ---------------------------------------------------------------------------
// "card-lookup" table — encoding D (columnar), ds-722 / task #7.
// ---------------------------------------------------------------------------
//
// The shipped CAS asset `card-lookup.<blake3>.bin` maps an engine lookup KEY →
// the card's `(uuid, version, dfc)`, from which both the client and `deepscry
...
/// Decode encoding D back into entries (for `deepscry download` + tests; the browser
...
/// task #7 steer #1). Native code (`deepscry download`) uses the same type.
#[derive(Debug, Clone, Default)]
pub struct CardLookupTable {
    /// key → (uuid bytes, version, dfc).
    map: std::collections::HashMap<String, ([u8; 16], u32, bool)>,
    /// Which index this table came from, and thus how its UUIDs reconstruct
    /// into URLs. Defaults to [`SCRYFALL_SCHEME`]; an AI-art table is decoded
    /// with [`CardLookupTable::from_bytes_with`] carrying its own scheme.
    scheme: UrlScheme,
}

impl CardLookupTable {
    /// Decode an encoding-D (SCDT) blob into a lookup table that reconstructs
    /// stock Scryfall URLs ([`SCRYFALL_SCHEME`]). `None` on a bad magic /
    /// unknown format / structural error (caller treats as a miss).
    pub fn from_bytes(bytes: &[u8]) -> Option<Self> {
        Self::from_bytes_with(bytes, SCRYFALL_SCHEME)
    }

    /// Decode an encoding-D (SCDT) blob into a lookup table whose UUIDs
    /// reconstruct against `scheme` (e.g. an R2-hosted AI-art index). `None` on
    /// a bad magic / unknown format / structural error (caller treats as a miss).
    pub fn from_bytes_with(bytes: &[u8], scheme: UrlScheme) -> Option<Self> {
        let entries = decode_card_lookup(bytes)?;
        let map = entries
            .into_iter()
            .map(|e| (e.key, (e.uuid, e.version, e.dfc)))
            .collect();
        Some(Self { map, scheme })
    }
    ...
    /// Resolve a card to its immutable CDN URL at `size` — using THIS table's
    /// own [`UrlScheme`] (stock Scryfall by default; the AI-art index uses its
    /// own host/ext) — or `None` on a table miss (the cascade then falls through
    /// to gatherer).
    ///
    ...
    pub fn cdn_url(
        ...
        let lookup = |key: &str| {
            self.map
                .get(key)
                .map(|(uuid, ver, _dfc)| cdn_image_url_with(self.scheme, &uuid_to_string(uuid), &ver.to_string(), size))
        };
        if is_token {
...
mod tests {
    ...
    #[test]
    fn cdn_url_art_crop_segment() {
        // Art-only mode (native web GUI) requests the SAME (uuid, version) at the
        // "art_crop" CDN size segment — the index needs no changes.
        assert_eq!(
            cdn_image_url("77c6fa74-5543-42ac-9ead-0e890b188e99", "1706239968", CdnSize::ArtCrop),
            "https://cards.scryfall.io/art_crop/front/7/7/77c6fa74-5543-42ac-9ead-0e890b188e99.jpg?1706239968",
        );
        assert_eq!(CdnSize::ArtCrop.segment(), "art_crop");
    }
    ...
    /// An R2-style scheme (custom host + `.png`) reconstructs the SAME UUID
    /// fan-out shape, only swapping `base` and `ext`. The default Scryfall
    /// scheme stays byte-identical (covered by the tests above).
    #[test]
    fn cdn_image_url_with_r2_scheme() {
        const R2_SCHEME: UrlScheme = UrlScheme {
            base: "https://cdn.deepscry.net/art_crop",
            ext: "png",
        };
        let id = "77c6fa74-5543-42ac-9ead-0e890b188e99";
        assert_eq!(
            cdn_image_url_with(R2_SCHEME, id, "9", CdnSize::ArtCrop),
            "https://cdn.deepscry.net/art_crop/art_crop/front/7/7/77c6fa74-5543-42ac-9ead-0e890b188e99.png?9",
        );
        // The DEFAULT wrapper is unchanged — same input, stock Scryfall .jpg URL.
        assert_eq!(
            cdn_image_url(id, "9", CdnSize::ArtCrop),
            "https://cards.scryfall.io/art_crop/front/7/7/77c6fa74-5543-42ac-9ead-0e890b188e99.jpg?9",
        );
        // SCRYFALL_SCHEME is exactly what cdn_image_url pins to.
        assert_eq!(
            cdn_image_url_with(SCRYFALL_SCHEME, id, "9", CdnSize::Small),
            cdn_image_url(id, "9", CdnSize::Small),
        );
        assert_eq!(UrlScheme::default(), SCRYFALL_SCHEME);
    }

    /// A `CardLookupTable` decoded with an R2 scheme resolves its cards to that
    /// scheme's host + `.png` — `…/<a>/<b>/<uuid>.png?<ver>` — while a table
    /// decoded the legacy way still yields stock Scryfall `.jpg` URLs.
    #[test]
    fn card_lookup_table_honors_its_scheme() {
        const R2_SCHEME: UrlScheme = UrlScheme {
            base: "https://cdn.deepscry.net/art_crop",
            ext: "png",
        };
